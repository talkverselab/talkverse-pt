...
| 번들 ID | `ios/Runner.xcodeproj/project.pbxproj` 의 `PRODUCT_BUNDLE_IDENTIFIER` (예: `com.talkverse.portugueseUniverse`). 앱마다 고유. |
...
`CFBundleDisplayName` 이 홈 화면 이름이다(예: 포르투갈어유니버스).
...
python -X utf8 tools/ios/make_ios_cert.py --key-id XXXXXXXXXX --issuer-id <UUID> --p8 AuthKey_XXXXXXXXXX.p8 \
    --bundle-id com.talkverse.portugueseUniverse --app-name "포르투갈어유니버스" --out ./ios_secrets
```
